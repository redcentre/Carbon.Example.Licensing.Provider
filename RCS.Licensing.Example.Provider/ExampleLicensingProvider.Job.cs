...
partial class ExampleLicensingProvider
{
	...
	public async Task<Job[]> ReadJobsByName(string jobName)
	{
		using var context = MakeContext();
	...
	public async Task<Job[]> ListJobs()
	{
		using var context = MakeContext();
		return await context.Jobs.AsNoTracking()
			.AsAsyncEnumerable()
	...
	public async Task<UpsertResult<Job>> UpsertJob(Job job)
	{
		using var context = MakeContext();
		EF.Job row;
		if (job.Id == null)
		...
		{
			EF.Job? oldrow = await context.Jobs.FirstOrDefaultAsync(j => j.Id.ToString() == job.Id).ConfigureAwait(false);
			if (oldrow == null)
			{
				return new UpsertResult<Job>(null, UpsertStatusCode.NotFound, $"Job Id {job.Id} not found for update");
			}
			row = oldrow;
		}
		row.Name = job.Name;
		...
		Job? job2 = await RereadJob(context, row.Id).ConfigureAwait(false);
		var result = new UpsertResult<Job>(job2, job.Id == null ? UpsertStatusCode.Inserted : UpsertStatusCode.Updated);
		if (job.Id == null)
		{
			// ┌───────────────────────────────────────────────────────────────┐
			// │  Attempt to create the Azure Container that is the backing    │
			// │  storage for a newly inserted Job.                            │
			// └───────────────────────────────────────────────────────────────┘
			if (row.CustomerId == null)
			{
				result.BackingStatus = BackingStatusCode.NoEntity;
				result.BackingMessage = $"Job Id {row.Id} has no parent Customer Id.";
			}
			else
			{
				var cust = await context.Customers.FirstOrDefaultAsync(c => c.Id == row.CustomerId).ConfigureAwait(false);
				if (cust == null)
				{
					result.BackingStatus = BackingStatusCode.NoEntity;
					result.BackingMessage = $"Job Id {row.Id} parent Customer Id {row.CustomerId} not found.";
				}
				else
				{
					if (cust.StorageKey == null)
					{
						result.BackingStatus = BackingStatusCode.NoConnection;
						result.BackingMessage = $"Job Id {row.Id} parent Customer Id {row.CustomerId} has no connection string.";
					}
					else
					{
						var sau = new StorageAccountUtility(cust.StorageKey);
						bool created = await sau.CreateContainer(row.Name);
						if (created)
						{
							result.BackingStatus = BackingStatusCode.Created;
							result.BackingMessage = $"Job Id {row.Id} Container {row.Name} was created.";
						}
						else
						{
							result.BackingStatus = BackingStatusCode.Exists;
							result.BackingMessage = $"Job Id {row.Id} Container {row.Name} already exists.";
						}
					}
				}
			}
	...
	public async Task<Job?> DisconnectJobChildUser(string jobId, string userId)
	{
	...
	public async Task<Job?> ConnectJobChildUsers(string jobId, string[] userIds)
	{
	...
	public Task<Job?> ReplaceJobChildUsers(string jobId, string[] userIds)
	{
	...
	public async Task<XElement> CompareJobsAndContainers()
	{
		...
		await foreach (var cust in context.Customers.AsNoTracking().Include(c => c.Jobs).AsAsyncEnumerable())
		{
			...
			try
			{
				...
				EF.Job[] jobonly = cust.Jobs.Where(j => !conlist.Any(c => c.Name == j.Name)).ToArray();
	...
	static async Task<Job?> RereadJob(EF.ExampleContext context, int jobId)
	{
